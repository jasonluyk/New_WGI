...
@app.post("/api/admin/sync-all-finals")
def admin_sync_all_finals(username: str = Depends(verify_admin)):
    """Triggers archive scrapes for all events that have a ShowID."""
    events = list(db["event_metadata"].find({"show_id": {"$ne": ""}}, {"_id": 0}))
    count = 0
    for event in events:
        show_id = event.get("show_id", "").strip()
        if not show_id:
            continue
        db["system_state"].insert_one({
            "type": "scraper_command",
        ...
        count += 1
    return {"message": f"Archive commands sent for {count} events."}
